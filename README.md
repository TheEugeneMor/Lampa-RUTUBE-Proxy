...
2. В открывшемся окне будут показаны адреса вида:
   ...
   ```text
   http://192.168.1.25:8787/rutube-cors.js
   ```

3. В Lampa добавьте URL с IP компьютера, который доступен устройству с Lampa. Обычно для телевизора или приставки в той же сети нужен адрес `http://192.168.x.x:8787/rutube-cors.js`.
...
```text
http://your-domain.example:8787/rutube-cors.js
```
